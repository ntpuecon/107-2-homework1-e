...
答:
case1.因為立足點不同，經原成績高的人對個體可能有比較高的學習能力，反之，經原成績低的人對於個體會相較低的學習能力
case2.會存在遺漏變數的因子，因可否休息個經除經原及格與否，也是會因為是否修習微積分且是否為0分有關，彼此之間可能會有不一樣的結論。

## 1.5 選擇性偏誤
這個資料還包含**經原PR**變數，它是學生最後修過的經原成績在該經原班的排名。說明有必要控制**經原PR**的理由（非數學證明）。
答:
因在探討控制經原PR之後，我們才能確定個經成績跟能不能修個體是獨立的，因為有可能經原成績高的通常個體成機會比較高，反之。
...
```{r ans16}
hw1Data %>%
  lm((個經PR)~(可修個體)+(經原PR),data=.)->model_2
summary(model_2)
coeftest(model_2, vcov. = vcovHC, type="HC1") -> model_2_coeftest
summary(model_2_coeftest)
...
```{r ans17}
library(tidyverse)
hw1Data %>% group_by(可修個體) %>% summarise(個體mea = mean(個經PR,na.rm=T),經原mea = mean(經原PR,na.rm=T))
```
...
請證明：
$\hat{\beta}_1=\bar{y}_1-\bar{y}_0,$其中$\bar{y}_1=\sum_{i,x_i=1}y_i/n_1$與$\bar{y}_0=\sum_{i,x_i=0}y_i/n_0$
分別為$x_i=1$與$x_i=0$兩群樣本的$y_i$平均。

答:
在$\hat{\beta}_1$方程式的分子形式可以轉寫為
...
而將分子分母兩式轉寫為的的$\hat{\beta}_1=\bar{y_1}-\bar{y_0}$
...
假設$E(\epsilon_i|x_i)=0$，證明上題設定下：
  ...
  答
  $\hat{\beta}=(x'x)^{-1}(x'y)=(x'x)^{-1}(\beta*x+\epsilon_i|x_i)=\beta+(x'x)^{-1}(\epsilon_i|x_i)=E(\hat{\beta})=\beta$
