...
$Y_{1i}-Y_{0i}$
此為可修和不可修個經的學生PR的差距
此處Y_{1i}為可修個經，並修完後的個經PR
    Y_{0i}為不可修個經，但修完後的個經PR   
...
此處Y_i為學生的個經PR
    Y_{0i}為不可修個經，但修完後的個經PR
    (Y_{1i}-Y_{0i})為可修和不可修個經的學生PR的差距
    T_i為可否修個經的虛擬變數，可修為1，不可修為0
...
請進行OLS估計前述的簡單迴歸模型。（注意估計式標準誤必需使用穩健標準誤robust standard
...
##1.4答:
1.因為立足點不同，經原成績高的人對個經可能有比較高的學習熱忱與能力，經原成績低的人可能比較沒有興趣學習。
2.有遺漏變數的因素，因為可否修個經除了經原及格與否，也跟是否修微積分，並是否為0分有關，有個人可能經原很好，但他微積分0分而導致他不能修個經。
...
##1.5答:
因為控制了經原PR之後，我們才能確定個經成績跟能不能修個經是獨立的，因為經原成績高的人通常個經表現會比較好，經原成績低的人可能表現會比較差。
...
## 1.7 變數關連
請計算**可修個體**為1與0兩群人的**經原PR平均**及**個經PR平均**。（hint: 可利用dplyr套件下的`group_by()`,`summarise()`，及`mean( ,經原PR,)`, na.rm=T表示計算時排除NA值） 

```{r ans17}
group_by(hw1Data,可修個體)%>%summarise(經原PR平均=mean(經原PR,na.rm=T),個經PR平均=mean(個經PR,na.rm=T))
...
請填入以下空格完成完整偏誤論述：  
有控制**經原PR**時，擋修會使得**個經PR** (1)下降(上升/下降) (2)6.802百分點，其值比未控制**經原PR**時還 (3)低(高/低)；這表示忽略**經原PR**會對效應係數估計產生 (4)正向偏誤(正向偏誤(upward bias)/負向偏誤(downward bias))。之所以會有上述的偏誤方向來自於兩個因素：
...
##2.1答:
在$\hat{\beta}_1$最小平方估計方程式的分子形式為
$${\sum_{i=1}^N (x_i-\bar{x})(y_i-\bar{y})}$$
=$$\sum_{i=1}^N (x_i-\bar{x})(y_i)$$
=$$\sum_{i=1}^Nx_iy_i-\sum_{i=1}^N \bar{x}y_i$$
=$$\frac{n_1n_0}{n_1+n_0}*(\bar{y_1}-\bar{y_0})$$
而分母形式為
$$\sum_{i=1}^N (x_i-\bar{x}^2)$$
=$$\sum_{i=1}^Nx_i^2-n\bar{x}$$
=$$n_1-n(\frac{n_1}{n_1+n_0})^2$$
=$$\frac{n_1n_0}{n_1n_0}$$
